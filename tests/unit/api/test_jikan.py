...
def test_fetch_data_success(
    ...
    params: dict[str, str | int | list[Any]] = {"page": 1}
...
def test_fetch_data_timeouts(
    ...
    mock_logger.error.assert_any_call(
        f"Request to {endpoint} with {params} timed out: {error_msg}"
    )
    mock_logger.error.assert_called_with(
        f"Failed to fetch data from {endpoint} with {params} after 3 attempts"
    )
...
def test_fetch_data_status_404(
    ...
    assert result == {}
    mock_logger.error.assert_called_with(
        f"Resource was not found for {endpoint} with {params}: {error_msg}"
    )
